...
def main(args):
    if args.modelpath is None:
        args.savepath = f'/root/autodl-tmp/logs/Projects/DCR/inferences/defaultsd/{args.dataset}'
    else:
        ...
        args.savepath = f'/root/autodl-tmp/logs/Projects/DCR/{new_folder_name}/laion_frozentext/{mp}'
        args.dataset = 'laion'
            
    if args.iternum is not None:
        args.savepath = f'{args.savepath}_{args.iternum}'
    # # caption style
    # args.savepath = f'{args.savepath}/{args.modelstyle}'
    if args.rand_noise_lam is not None:
        args.savepath = f'{args.savepath}_ginfer{args.rand_noise_lam}'
    if args.rand_augs is not None:
        args.savepath = f'{args.savepath}_auginfer_{args.rand_augs}_{args.rand_aug_repeats}'
    os.makedirs(args.savepath,exist_ok=True)
    os.makedirs(f'{args.savepath}/generations',exist_ok=True)
    if args.modelpath is None:
        checkpath = "/root/autodl-tmp/stable-diffusion-2-1/"
    ...
    if args.modelstyle == "nolevel":
        ...
        if args.dataset == 'imagenette10':
            ...
            if args.modelstyle == 'instancelevel_blip':
                prompt_json = './miscdata/laion_combined_captions.json'
            else:
                raise Exception('Case not written')
    ...
    with open(f'{args.savepath}/prompts.txt', 'w') as f:
    ...
    for i in tqdm(range(num_batches), desc='Processing batches', unit='batch'):
        ...
        for j in range(len(images)):
            ...
            image.save(f"{args.savepath}/generations/{count}.png")
            count+=1
