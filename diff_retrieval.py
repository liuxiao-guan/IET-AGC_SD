    ...
    def __init__(self, main_dir, transform, capjson=None):
        ...
        if not os.path.exists(f"{main_dir}/prompts.txt"): # TODO: will not work for imagenettemodel
            ...
            if 'laion' in main_dir or 'l100kaion' in main_dir:
                capjson = glob.glob(os.path.join(p, "*combined_captions_modify.json"))[0]
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.rank == 0:  # only rank 0 will work from now on
        ...
        plt.savefig(f"{ret_savepath}/simplicityscatter_entropies.png")
        plt.figure()
        ...
        plt.savefig(f"{ret_savepath}/simplicityscatter_tvls.png")
        plt.figure()
        ...
        plt.savefig(f"{ret_savepath}/simplicityscatter_crs.png")
        plt.figure()
       ...
       ## simscores of repeated images
        if "nodup" not in args.query_dir:
            ...
            plt.savefig(f"{ret_savepath}/weightplot.png")
            # plt.close(fig)
            plt.figure()
        ...
        with open(f"{ret_savepath}/output.txt", "w") as file:
            ...
            print(f"sim_90pc:{sim_90pc}", file=file)
            print(f"sim_95pc:{sim_95pc}", file=file)
            print(f"clipscore:{clipscore}", file=file)
            print(f"fid:{fid}", file=file)
        ...
        for nm in range(0,showtill + 10,10):
            ...
            plt.imshow(result)
            plt.axis('off')
            os.makedirs(f'{ret_savepath}/',exist_ok=True)
            plt.savefig(f'{ret_savepath}/{nm}.png',bbox_inches='tight')
            plt.close()
